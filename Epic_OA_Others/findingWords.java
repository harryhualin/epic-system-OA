 ...
 public class findingWords{
 	...
 	public static boolean findWord(String word, char[][] board, int pos, int i, int j){
 	  ...
 	  if(board[i][j] == word.charAt(pos)){
 	  	board[i][j] ^= 256;
 	  	if(findWord(word,board,pos+1,i+1,j) || findWord(word,board,pos+1,i-1,j) || findWord(word,board,pos+1,i,j+1) || findWord(word,board,pos+1,i,j-1) || findWord(word,board,pos+1,i+1,j+1) || findWord(word,board,pos+1,i-1,j-1) || findWord(word,board,pos+1,i+1,j-1) || findWord(word,board,pos+1,i-1,j+1)){
 	  		return true;
 	  	}
 	  	board[i][j] ^= 256;
 	  }
